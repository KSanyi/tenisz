...
class KTRHistoryChart extends ApexCharts {

    KTRHistoryChart(KTRHistory ktrHistory) {
        ...
        for(var entry : ktrHistory.entries()) {
            seriesData.add(new Coordinate<>(entry.date().toString(), entry.ktr().value()));
        }
        super.setSeries(new Series<>("KTR", seriesData.toArray(new Coordinate[0])));
    }
    
    public void setData(KTRHistory ktrHistory) {
        List<Coordinate<?,?>> seriesData = new ArrayList<>();
        for(var entry : ktrHistory.entries()) {
            seriesData.add(new Coordinate<>(entry.date().toString(), entry.ktr().value()));
        }
